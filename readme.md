...
- .NET SDK 8.0 (recommended)
	- The project also multi-targets .NET 9.0; use an appropriate SDK if you prefer net9.0.
- SQLite (no separate server needed; DB files are local)
...
- `api/www/api/**` – pre-generated static assets copied from the main project (do not edit)

## Configuration

Configuration is provided via `appsettings.json` and `appsettings.Development.json`. Database file locations are bound via options objects used in endpoints (e.g., `AnimeDbOptions`, `AltAnimeDbOptions`) and EF Core contexts for users and API data.

SQLite database files expected by the app:

- `anime-dimension.db` – upstream Anime-Planet data snapshot (from the Database Orchestrator; not checked into git)
- `anime.db` – ancillary data (currently used for notifications; may be phased out)
- `users.db` – users and sessions (not in git)

Place these files at the project root (same folder as `ASP.NETCoreWebApi.csproj`) for local runs. The build/publish steps copy the DBs to output as needed (see the `.csproj`).
